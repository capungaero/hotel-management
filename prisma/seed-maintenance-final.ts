...
async function main() {
  ...
  // Create maintenance categories (check if they exist first)
  const existingCategories = await prisma.maintenanceCategory.findMany()
  let maintenanceCategories = existingCategories

  if (existingCategories.length === 0) {
    maintenanceCategories = await Promise.all([
    ...
    console.log('✅ Maintenance categories already exist')
  }

  // Create staff (check if they exist first)
  const existingStaff = await prisma.staff.findMany()
  let maintenanceStaff = existingStaff.filter(s => s.department === 'maintenance')
  let housekeepingStaff = existingStaff.filter(s => s.department === 'housekeeping')

  if (maintenanceStaff.length === 0) {
    maintenanceStaff = await Promise.all([
      prisma.staff.create({
        data: {
          name: 'John Smith',
          email: 'john.smith.maintenance@hotel.com',
          phone: '+1234567890',
      ...
      prisma.staff.create({
        data: {
          name: 'Mike Johnson',
          email: 'mike.johnson.maintenance@hotel.com',
          phone: '+1234567891',
    ...
    console.log('✅ Created maintenance staff')
  } else {
    console.log('✅ Maintenance staff already exist')
  }

  if (housekeepingStaff.length === 0) {
    housekeepingStaff = await Promise.all([
      prisma.staff.create({
        data: {
          name: 'Sarah Wilson',
          email: 'sarah.wilson.housekeeping@hotel.com',
          phone: '+1234567892',
      ...
      prisma.staff.create({
        data: {
          name: 'Maria Garcia',
          email: 'maria.garcia.housekeeping@hotel.com',
          phone: '+1234567893',
          position: 'Housekeeper',
          department: 'housekeeping',
      ...
      prisma.staff.create({
        data: {
          name: 'Lisa Chen',
          email: 'lisa.chen.housekeeping@hotel.com',
          phone: '+1234567894',
    ...
    console.log('✅ Created housekeeping staff')
  } else {
    console.log('✅ Housekeeping staff already exist')
  }

  // Create housekeeping tasks (check if they exist first)
  const existingHousekeepingTasks = await prisma.housekeepingTask.findMany()
  let housekeepingTasks = existingHousekeepingTasks

  if (existingHousekeepingTasks.length === 0) {
    housekeepingTasks = await Promise.all([
  ...
  const existingMaintenanceTasks = await prisma.maintenanceTask.findMany()
  ...
  if (existingMaintenanceTasks.length === 0) {
    ...
    console.log('✅ Maintenance tasks already exist')
  }

  // Create some sample housekeeping assignments for today
  const existingAssignments = await prisma.housekeepingAssignment.findMany()
  ...
  if (existingAssignments.length === 0) {
    ...
    console.log('✅ Housekeeping assignments already exist')
  }
